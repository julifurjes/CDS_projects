...
def training(datagen, train_dataset, test_dataset, y_train, y_test):
    ...
    H = model.fit(datagen.flow(train_dataset, y_train, batch_size=500), 
              validation_data = datagen.flow(test_dataset, y_test, batch_size=500, subset = "validation"),
            epochs=1)
    return H
...
def main():
    ...
    H = training(datagen, train_dataset, test_dataset, y_train, y_test)
